...
| `~/.local/share/junie-local/server-config.json` | persistent server settings — model/drafter, context length, KV quantization, auto-unload, plus launch settings (host/port, prefill tuning, seed request); created with defaults on first start, updated by `POST /apply_settings`, hand-editable while the server is stopped |
